...
export default function TabBarItem({
  ...
  const isPressed = useSharedValue(false); // JSON.parse(JSON.stringify is due to an issue with reanimated

  const sharedLabelStyle = useSharedValue(JSON.parse(JSON.stringify(labelStyle)));
  const sharedSelectedLabelStyle = useSharedValue(JSON.parse(JSON.stringify(selectedLabelStyle))); // NOTE: We clone these color values in refs because they might contain a PlatformColor value
  ...
  return (// @ts-expect-error caused by @types/react v18 and should be fixed by gesture-handler lib
    <GestureDetector gesture={gesture}>
      <View reanimated // @ts-expect-error
