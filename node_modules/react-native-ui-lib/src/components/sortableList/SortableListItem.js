...
const SortableListItem = props => {
  ...
  useDidUpdate(() => {
    ...
    initialIndex.value = _map(data, 'id').indexOf(id);
  }, [data]);
  useAnimatedReaction(() => itemsOrder.value, (currItemsOrder, prevItemsOrder) => {
  ...
  const onDragUpdate = useCallback(event => {
    ...
    translateY.value = tempTranslateY.value + event.translationY; // Swapping items
  ...
  const {
    dragAfterLongPressGesture,
    isFloating
  } = useDragAfterLongPressGesture({
    onDragStart,
    onDragUpdate,
    onDragEnd,
  ...
  const draggedAnimatedStyle = useAnimatedStyle(() => {
    ...
    const shadow = isFloating.value ? { ...Shadows.sh30.bottom,
      ...Shadows.sh30.top
    } : {
  ...
  return (// @ts-expect-error related to children type issue that started on react 18
